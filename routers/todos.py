...
async def read_all_by_user(request: Request, db: Session = Depends(get_db)):
    ...
    todos = db.query(models.Todos).filter(models.Todos.owner_id == user.get('user_id')).order_by(models.Todos.id).all()
...
async def del_todo(request: Request, todo_id: int, db: Session = Depends(get_db)):
    user = await get_current_user(request)
    if user is None:
        return RedirectResponse(url='/auth', status_code=status.HTTP_302_FOUND)
